Shader "urpshaders/character/customized/character_toonlit_customized_kalienina_hair"
{
    Properties
    {
        ...
        _SpecularSettings( "# 【高光】设置", Float) = 1
            _SpecularRange("【高光】大小",Range(0,5)) =1
            _SpecularIntensity("【高光】亮部强度",Range(0,5)) =1
            _SpecularDarkIntensity("【高光】暗部强度",Range(0,5)) =1
            _SpecularNVExp("【高光】视角衰减1",Float) =1
            _SpecularNVIntensity("【高光】视角衰减2",Float) =1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            CBUFFER_START(UnityPerMaterial)
                ...
                half _SpecularDarkIntensity;
                half _SpecularNVExp;
                half _SpecularNVIntensity;
                half _RimWidth;
                half _RimIntensity;
                half _OutLineWidth;
            CBUFFER_END
            ...
            float4 frag(ToonVaryings input):SV_Target
            {
                ...
                float Threshold = step(_RampOffset, NL) * ShadowAO;
                float3 diffuse = lerp(DarkSide, BrightSide, Threshold);

                //高光
                float specularMask = lightMap.r;
                float specularNVThreshold = pow(1-NV,_SpecularNVExp)* _SpecularNVIntensity;
                float specularBase = specularMask* _SpecularRange;
                float3 specular = step(specularNVThreshold, specularBase)*(specularMask>0.1);
